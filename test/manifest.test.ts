...
// build enforces (see docs/app-manifest.md). This app takes no settings and does
// no rotation or data refresh, so — like the Quotes app — it carries neither a
// `settings` block nor `playback`, and `launch` is just the base URL.

const BASE_URL = 'https://capitals.srly.io/'
...
describe('signage-app manifest', () => {
  ...
  test('is a single-shot page with nothing to pace', () => {
    // Fresh quiz per load, no rotation loop, static data — no pacing to describe.
